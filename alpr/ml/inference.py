...
def run_ocr(image_path, brightness=0, contrast=0):
    ...
    # 1) Intento "normal": respetando el padding (quita ceros finales)
    plate_text = decode_indices(indices)

    # 2) Fallback: si quedó vacío (todo ceros), armamos igual el string completo
    #    usando el alfabeto directamente. Así SIEMPRE devolvemos algo.
    if not plate_text:
        plate_text = "".join(ALPHABET[int(i)] for i in indices)
